...
class IQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        four_directions = [[-9, 0, 9], [-1, 0, 1], [-10, 0, 10], [-8, 0, 8]]
        SAI_name = self.data_dict['d_img_list'][idx]
        SAI_no = int(SAI_name.split('_')[-1][:2])
        SAI_name_cut = '_'.join(SAI_name.split('_')[:-1])
        res = []
        for d in range(4):
            SAI_list = [SAI_name_cut + '_' + str(four_directions[d][i] + SAI_no) + '.bmp' for i in range(len(four_directions[d]))]
        ...
        sample = {'vertical': res[0], 'horizontal': res[1], 'left': res[2], 'right': res[3], 'score': score}
