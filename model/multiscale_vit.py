...
class MS_IQAregression(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = Multi_scale_VisionTransformer(self.config)
    ...
    def forward(self, mask_inputs, feat_dis_org, feat_dis_scale_1, feat_dis_scale_2):
        ...
        feat_dis_scale_1 = self.conv_enc(feat_dis_scale_1)
        # print("feat_dis_scale_1", feat_dis_scale_1.size())
        feat_dis_scale_2 = self.conv_enc(feat_dis_scale_2)
        # print("feat_dis_scale_2", feat_dis_scale_2.size())

        # （batch_size, n_enc_seq+1, d_hidn) -> (batch_size, d_hidn)
        enc_outputs = self.transformer(mask_inputs, feat_dis_org, feat_dis_scale_1, feat_dis_scale_2)
...
class Multi_scale_VisionTransformer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class Encoder(nn.Module):
    ...
    def forward(self, mask_inputs, feat_dis_org_embed, feat_dis_scale_1_embed, feat_dis_scale_2_embed):
        ...
        # feat_dis_scale_1_embed : batch_size x (C = 384) x (h = 384 * H / W / 32 = 12 * H / W) x (w = 384 / 32)
        # feat_dis_scale_2_embed : batch_size x (C = 384) x (h = 224 * H / W / 32 = 7 * H / W)x (w = 224 / 32)
        ...
        scale_2_embed = repeat(self.scale_2_embedding, '() c () () -> b c h w', b=self.config.batch_size, h=5, w=7)
        ...
        # print("inputs_embed=>", inputs_embed.size())
